...
export class ToolTransformer {
  ...
  static convertTools(claudeTools: ClaudeTool[]): ToolConversionResult {
    ...
    // 检查是否有WebSearch工具（名称匹配）
    const hasWebSearchTool = supportedTools.some(tool =>
      tool.name === 'WebSearch' || tool.name === 'web_search'
    );

    if (hasWebSearchTool) {
      // 注意：Gemini的google_search不能与function工具同时使用
      // 所以当有WebSearch时，需要将其作为普通函数工具处理
      // 暂时保留所有工具作为函数工具，包括WebSearch
    }

    // 保留所有工具作为函数工具
    if (supportedTools.length > 0) {
      const functionDeclarations: GeminiFunctionDeclaration[] = supportedTools.map(tool => {
        // 规范化工具名称以符合 Gemini API 要求
  ...
  /**
   * 规范化工具名称以符合 Gemini API 要求
   * Gemini 要求：必须以字母或下划线开头，只能包含字母数字、下划线、点、冒号或破折号
   */
  static normalizeToolName(name: string): string {
    if (!name) return 'tool';
