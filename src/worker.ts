  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (method === 'GET' && pathname === '/logs') {
        ...
        const hasError = queryParams.hasError === 'true' ? true : queryParams.hasError === 'false' ? false : undefined;
        const isStream = queryParams.isStream === 'true' ? true : queryParams.isStream === 'false' ? false : undefined;

        const logs = Logger.getAllLogs({ limit, offset, hasError, isStream });
      ...
      if (method === 'GET' && pathname.startsWith('/logs/')) {
        const logRequestId = pathname.split('/')[2];
        const headers = corsEnabled
          ? createResponseHeaders('application/json')
          : new Headers({ 'Content-Type': 'application/json', 'Cache-Control': 'no-cache' });

        const log = Logger.getLog(logRequestId);
        if (!log) {
          return new Response(
        ...
        return new Response(
          JSON.stringify({ log, timestamp: new Date().toISOString() }),
          { status: 200, headers }
        );
      }
